...
#         cols = agg_gdf.columns.tolist()
#         if 'geometry' in cols:
#             cols.remove('geometry')
#         agg_gdf.columns = ['h3_res'] + ['_'.join(col).strip() if isinstance(col, tuple) else col for col in cols[1:]] + ['geometry']
...
#         if 'geometry' in cols:
#             cols.remove('geometry')
#         agg_gdf.columns = ['h3_res'] + [('_'.join(col).strip() + '_rent') if isinstance(col, tuple) else col for col in cols[1:]] + ['geometry']
#         agg_gdf = agg_gdf.set_geometry('geometry')
#         metric_choices = [c for c in agg_gdf.columns if c not in ['h3_res', 'geometry']]
